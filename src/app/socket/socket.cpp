...
public:
    ...
    void post_run(Instance *xsock)
    {
        ...
        log(info) << "shutdown SocketServer";
    ...
    void print_run(Instance *xsock)
    {
        ...
        if (rstatus < 0) {
		    log(info) << "socket - started on pid " << xsock->pid;
	    } else {
		    log(info) << "socket - "<< COMPILE_DATE << " " << COMPILE_TIME << " built for " <<
				name.sysname << " " << name.release << " " << name.machine << " started on pid " << xsock->pid;
	    }
        log(info) << "socket pid filename[" <<  xsock->pid_filename << "]";
    }
...
int main(int argc, char **argv)
{
	...
	if (base.show_version) {
        std::cout << "socket built: " << COMPILE_DATE << " " << COMPILE_TIME << std::endl;
        std::cout << "socket version: " << SOCKET_VERSION << std::endl;
        if (base.show_help) {
            base.show_usage();
        }
        exit(0);
